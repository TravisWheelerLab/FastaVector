...
void readTest(char ** const testHeaderStrings, char ** const testSequenceStrings,
  ...
  for(size_t i = 0; i < numHeaders; i++){
    ...
    sprintf(buffer, "header length of %zu did not match expected length %zu.", headerLength, expectedHeaderLength);
    ...
    sprintf(buffer, "sequence length of %zu did not match expected length %zu.", sequenceLength, expectedSequenceLength);
