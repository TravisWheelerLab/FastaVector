...
void testInitSequenceList() {
  ...
  testAssertString(
      fastaVector.sequence.capacity ==
          FASTA_VECTOR_CHAR_VECTOR_DEFAULT_CAPACITY,
      "sequence vector capacity was not the expected default value after init");
  testAssertString(
  ...
  testAssertString(
      fastaVector.metadata.capacity ==
          FASTA_VECTOR_SEQUENCE_METADATA_VECTOR_DEFAULT_CAPACITY,
      "metadata vector capacity was not the expected default value after init");
