...
void testInsertSequences() {
    ...
    for (size_t i = 0; i < numSequences; i++) {
        ...
        for (size_t sequencePos = 0; sequencePos < sequenceLength;
             sequencePos++) {
    ...
    for (size_t i = 0; i < numSequences; i++) {
        ...
        sprintf(buffer,
                ...
                "FASTA_VECTOR_OK. \nheader: %.*s, \n seq: %.*s",
                i, (int)headerLengths[i], headerPtrs[i],
                (int)sequenceLengths[i], sequencePtrs[i]);
    ...
    for (size_t i = 0; i < numSequences; i++) {
        ...
        size_t headerEndPosition =
            fastaVector.metadata.data[i].headerEndPosition;
        ...
        for (size_t i = 0; i < numSequences; i++) {
            size_t sequenceStartPosition =
                i == 0 ? 0
                       : fastaVector.metadata.data[i - 1].sequenceEndPosition;
            char *mainSequencePtr = fastaVector.sequence.charData;
            sprintf(buffer,
                    "sequence number %zu did not match what was stored.", i);
            bool stringsMatch = strncmp(sequencePtrs[i],
                                        mainSequencePtr + sequenceStartPosition,
                                        sequenceLengths[i]) == 0;
            ...
            sprintf(buffer, "header number %zu did not match what was stored.",
                    i);
            stringsMatch =
                strncmp(headerPtrs[i], mainHeaderPtr + headerStartPosition,
                        headerLengths[i]) == 0;
            testAssertString(stringsMatch, buffer);
        }
    }
