...
void fastaVectorWriteStaticTest() {
    ...
    char *header2 = "2";
    char *sequence2 =
        "wtreyeuioytrewdrfghjgfdsafghfdgbhdgv1234567890-=!@#$%^&*()"
...
void fastaVectorFileWriteTest(const size_t numSequences, const char *fileSrc) {
    ...
    for (size_t sequenceNum = 0; sequenceNum < numSequences; sequenceNum++) {
        ...
        fastaVectorFastaGetSequence(&fastaVector, sequenceNum,
                                    &sequenceFromVector,
        ...
        sprintf(buffer, "header was supposed to match %s, but got %.*s.",
                headerPtr, (int)headerLengthFromVector, headerFromVector);
        testAssertString(
            strncmp(headerFromVector, headerPtr, headerLength) == 0, buffer);
        sprintf(buffer, "sequence was supposed to be length %zu, but got %zu.",
                sequenceLength, sequenceLengthFromVector);
        testAssertString((sequenceLength + 1) == sequenceLengthFromVector,
                         buffer);
        sprintf(buffer, "sequence was supposed to match %s, but got %.*s.",
                sequencePtr, (int)sequenceLengthFromVector, sequenceFromVector);
        testAssertString(
            strncmp(sequenceFromVector, sequencePtr, sequenceLength) == 0,
            buffer);
    }
    ...
    if (fastaVector.header.count != fastaReadVector.header.count) {
        ...
        printf("headers did not match, \n h1: %.*s\n h2: %.*s\n hr: %.*s\n",
               ...
               (int)fastaReadVector.header.count,
               fastaReadVector.header.charData, (int)lastHeaderLen,
               lastHeaderPtr);
    }
    sprintf(
        buffer,
    ...
    testAssertString(
        fastaVector.sequence.count == fastaReadVector.sequence.count, buffer);
    sprintf(buffer,
    ...
    testAssertString(
        fastaVector.metadata.count == fastaReadVector.metadata.count, buffer);
    sprintf(buffer,
    ...
    for (size_t headerCharIndex = 0;
        ...
        sprintf(buffer,
                "header char @ pos %zu did not match, write: %c, read %c.",
    ...
    for (size_t metadataIndex = 0;
         metadataIndex < fastaReadVector.metadata.count; metadataIndex++) {
        sprintf(buffer,
        ...
        testAssertString(
            fastaVector.metadata.data[metadataIndex].sequenceEndPosition ==
                fastaReadVector.metadata.data[metadataIndex]
                    .sequenceEndPosition,
            buffer);
    }
