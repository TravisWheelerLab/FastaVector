...
void readTest(char **const testHeaderStrings, char **const testSequenceStrings,
    ...
    for (size_t i = 0; i < numHeaders; i++) {
        ...
        const size_t expectedSequenceLength =
            strlen(testSequenceStrings[i]) + 1;
        size_t headerLength;
        char *headerPtr;
        fastaVectorFastaGetHeader(&fastaVector, i, &headerPtr, &headerLength);
        sprintf(buffer,
                "header length of %zu did not match expected length %zu.",
        ...
        fastaVectorFastaGetSequence(&fastaVector, i, &sequencePtr,
                                    &sequenceLength);
        sprintf(buffer,
                "sequence length of %zu did not match expected length %zu.",
...
int main(int argc, char **argv) {
    ...
    testHeaderStrings[0] = "test 2 header ";
    testSequenceStrings[0] =
        "awldeifbawiefbawlieskfbalkhsdbfalkhsbdflkhasbdflhjk"
    ...
    testSequenceStrings[2] =
        "seq3Dataseq3Dataseq3Dataseq3Dataseq3Dataseq3Dataseq"
