...
bool fastaVectorMetadataVectorAddMetadata(struct FastaVectorMetadataVector *vector){
  ...
  if(__builtin_expect(vector->count == 0, false)){
    ...
    vector->data[vector->count].sequenceEndPosition = vector->data[vector->count - 1].sequenceEndPosition;
    vector->data[vector->count].headerEndPosition = vector->data[vector->count - 1].headerEndPosition;
  }
...
bool fastaVectorMetadataVectorResize(struct FastaVectorMetadataVector *vector){
  ...
  vector->data = realloc(vector->data, newCapacity * sizeof(struct FastaVectorMetadata));
